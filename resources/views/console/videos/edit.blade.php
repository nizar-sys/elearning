        ...
        <div class="card">
            ...
            <div class="card-header border-bottom">
                <h5 class="card-title mb-0">
                    <span class="fw-normal">Edit Video</span>
                </h5>
            </div>
            ...
            <div class="card-body">
                <div class="offcanvas-body mx-0 flex-grow-0 h-100 mt-2">
                    ...
                    <form class="video pt-0" id="videoForm" method="POST" action="{{ route('videos.update', $video->id) }}"
                                    ...
                                    <select class="form-control @error('created_by') is-invalid @enderror"
                                        ...
                                        @foreach ($creators as $creator)
                                            <option value="{{ $creator->id }}"
                                                {{ old('created_by', $video->created_by) == $creator->id ? 'selected' : '' }}>
                                                {{ $creator->name }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="col-sm-12">
                                ...
                                <div class="form-floating form-floating-outline mb-5 mt-2">
                                    <textarea class="form-control @error('description') is-invalid @enderror full-editor" id="video-description"
                                        name="description" aria-label="Description" placeholder="Description" cols="30" rows="10">{{ old('description', $video->description) }}</textarea>
                                    @error('description')
                                        <div class="invalid-feedback d-block">{{ $message }}</div>
                                    @enderror
                                </div>
                                ...
                                <div class="form-floating form-floating-outline mb-5 mt-5">
                                    <select id="category_id[]" class="select2 form-select" data-placeholder="All Categories"
                                        ...
                                        @foreach ($categories as $category)
                                            <option value="{{ $category->id }}" @if (in_array($category->id, old('category_id', $video->categories->pluck('id')->toArray()))) selected @endif>{{ $category->name }}</option>
                                        @endforeach
                                    </select>
...
@push('scripts')
    <script>
        var existingFiles = @json($video->thumbnail);
    </script>
    @vite('resources/js/console/videos/edit_validation_script.js')
@endpush
