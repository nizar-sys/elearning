        ...
        <div class="card">
            ...
            <div class="card-header border-bottom">
                <h5 class="card-title mb-0">
                    <span class="fw-normal">Edit Article</span>
                </h5>
            </div>
            ...
            <div class="card-body">
                <div class="offcanvas-body mx-0 flex-grow-0 h-100 mt-2">
                    ...
                    <form class="article pt-0" id="articleForm" method="POST"
                                    ...
                                    <select class="form-control @error('created_by') is-invalid @enderror"
                                        ...
                                        @foreach ($creators as $creator)
                                            <option value="{{ $creator->id }}"
                                                {{ old('created_by', $article->created_by) == $creator->id ? 'selected' : '' }}>
                                                {{ $creator->name }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="col-sm-12">
                                ...
                                <div class="form-floating form-floating-outline mb-5 mt-2">
                                    <textarea class="form-control @error('content') is-invalid @enderror full-editor" id="article-content" name="content"
                                        aria-label="Content" placeholder="Content" cols="30" rows="10">{{ old('content', $article->content) }}</textarea>
                                    @error('content')
                                        <div class="invalid-feedback d-block">{{ $message }}</div>
                                    @enderror
                                </div>
...
@push('scripts')
    @vite('resources/js/console/articles/edit_script.js')
    <script>
        var existingFiles = @json($article->thumbnail); // for dropzone
    </script>
@endpush
