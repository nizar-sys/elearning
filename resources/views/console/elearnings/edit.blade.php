        ...
        <div class="card">
            ...
            <div class="card-header border-bottom">
                <h5 class="card-title mb-0">
                    <span class="fw-normal">Edit Elearning</span>
                </h5>
            </div>
            ...
            <div class="card-body">
                <div class="offcanvas-body mx-0 flex-grow-0 h-100 mt-2">
                    ...
                    <form class="elearning pt-0" id="elearningForm" method="POST"
                        action="{{ route('elearnings.update', $elearning->id) }}" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                                    ...
                                    <select class="form-control @error('teacher_id') is-invalid @enderror"
                                        ...
                                        @foreach ($teachers as $teacher)
                                            <option value="{{ $teacher->id }}"
                                                {{ old('teacher_id', $elearning->teacher_id) == $teacher->id ? 'selected' : '' }}>
                                                {{ $teacher->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                    ...
                                    <select class="form-control @error('benefit_id') is-invalid @enderror"
                                        ...
                                        @foreach ($benefits as $benefit)
                                            <option value="{{ $benefit->id }}"
                                                {{ old('benefit_id', $elearning->benefit_id) == $benefit->id ? 'selected' : '' }}>
                                                {{ $benefit->type }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="col-sm-12 col-md-6">
                                <div class="form-floating form-floating-outline mb-5 mt-2">
                                    <input type="text" class="form-control @error('title') is-invalid @enderror"
                                        ...
                                        value="{{ old('title', $elearning->title) }}" />
                            ...
                            <div class="col-sm-12 col-md-6">
                                <div class="form-floating form-floating-outline mb-5 mt-2">
                                    <input type="text" class="form-control @error('duration') is-invalid @enderror"
                                        ...
                                        value="{{ old('duration', $elearning->duration) }}" />
                            ...
                            <div class="col-sm-12">
                                ...
                                <div class="form-floating form-floating-outline mb-5 mt-2">
                                    <textarea class="form-control @error('description') is-invalid @enderror full-editor" id="elearning-description"
                                        name="description" aria-label="Description" placeholder="Description" cols="30" rows="10">{{ old('description', $elearning->description) }}</textarea>
                                    @error('description')
                                        <div class="invalid-feedback d-block">{{ $message }}</div>
                                    @enderror
                                </div>
                            ...
                            <div class="col-sm-12 col-md-6">
                                ...
                                <div class="form-floating form-floating-outline mb-5 mt-5">
                                    <select id="material_id[]" class="select2 form-select" data-placeholder="All Materials"
                                        ...
                                        @foreach ($materials as $material)
                                            <option value="{{ $material->id }}" @if (in_array($material->id, old('material_id', $elearning->materials->pluck('id')->toArray()))) selected @endif>{{ $material->title }}</option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="form-floating form-floating-outline mb-5 mt-5">
                                    <select id="category_id[]" class="select2 form-select"
                                        data-placeholder="All Categories" name="category_id[]" multiple>
                                        <option value="">All Categories</option>
                                        @foreach ($categories as $category)
                                            <option value="{{ $category->id }}"
                                                @if (in_array($category->id, old('category_id', $elearning->categories->pluck('id')->toArray()))) selected @endif>{{ $category->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="form-floating form-floating-outline mb-5 mt-3">
                                    <select class="form-control @error('status') is-invalid @enderror"
                                        id="elearning-status" name="status" aria-label="Status">
                                        <option value="" disabled selected>Select Status</option>
                                        @foreach ($elearningStatus as $status)
                                            <option value="{{ $status }}"
                                                {{ old('status', $elearning->status) == $status ? 'selected' : '' }}>
                                                {{ ucfirst($status) }}
                                            </option>
                                        @endforeach
                                    </select>
...
@push('scripts')
    @vite('resources/js/console/elearnings/edit_validation_script.js')
    <script>
        var existingFiles = @json($elearning->thumbnail);
    </script>
@endpush
