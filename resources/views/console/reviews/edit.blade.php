        ...
        <div class="card">
            ...
            <div class="card-header border-bottom">
                <h5 class="card-title mb-0">
                    <span class="fw-normal">Edit Review</span>
                </h5>
            </div>
                                    ...
                                    <select class="form-control @error('elearning_id') is-invalid @enderror"
                                        ...
                                        @foreach ($elearnings as $elearning)
                                            <option value="{{ $elearning->id }}"
                                                {{ old('elearning_id', $review->elearning_id) == $elearning->id ? 'selected' : '' }}>
                                                {{ $elearning->title }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="col-sm-12 col-md-12">
                                ...
                                <div class="form-floating form-floating-outline mb-5 mt-2">
                                    <textarea class="form-control @error('review') is-invalid @enderror full-editor" id="review-content"
                                        placeholder="Write your review" name="review" aria-label="Review" cols="30" rows="5">{{ old('review', $review->review) }}</textarea>
                                    @error('review')
                                        <div class="invalid-feedback d-block">{{ $message }}</div>
                                    @enderror
                                </div>
...
@push('scripts')
    <script>
        var ratingExists = "{{ $review->rating }}";
    </script>
