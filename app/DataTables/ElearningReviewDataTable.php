...
class ElearningReviewDataTable extends DataTable
{
    /**
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addIndexColumn()
            ->addColumn('action', 'console.reviews.action')
            ->editColumn('review', function ($query) {
                return str()->limit($query->review, 50);
            })
     ...
     */
    public function query(ElearningReview $model): QueryBuilder
    {
        return $model->newQuery()->with('elearning', 'reviewer');
    }
    ...
    public function html(): HtmlBuilder
    {
        ...
        $buttons = [
            [
                'text' => '<i class="ri-add-line me-0 me-sm-1"></i><span class="d-none d-sm-inline-block">Add Review</span>',
                'className' => 'add-new btn btn-primary mb-5 mb-md-0 me-3 waves-effect waves-light',
                'init' => 'function (api, node, config) {
                    $(node).removeClass("btn-secondary");
                }',
                'action' => 'function (e, dt, node, config) {
                    window.location = "' . route('reviews.create') . '";
                }'
            ],
            [
                ...
                'action' => 'function (e, dt, node, config) {
                    $("#reviews-table_filter input").val("").keyup();
                    dt.draw();
                }'
            ]
        ];
        ...
        return $this->builder()
            ...
            ->ajax([
                ...
                'type' => 'GET',
                'data' => "function(d){
                    d.elearning_id = $('#elearning_id_filter').val();
                    d.reviewer_id = $('#reviewer_id_filter').val();
                }",
            ]);
    }
    ...
    public function getColumns(): array
    {
        return [
            ...
            Column::make('elearning.title')->title('Elearning Title'),
            Column::make('reviewer.name')->title('Reviewer Name'),
            Column::make('review'),
            Column::make('rating'),
    ...
    {
        return 'ElearningReview_' . date('YmdHis');
    }
}
