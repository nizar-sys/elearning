...
class MaterialDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        ...
        return (new EloquentDataTable($query))
            ->addIndexColumn()
            ->addColumn('action', 'console.materials.action')
    ...
    public function html(): HtmlBuilder
    {
        ...
        $buttons = [
            [
                'text' => '<i class="ri-add-line me-0 me-sm-1"></i><span class="d-none d-sm-inline-block">Add Material</span>',
                'className' => 'add-new btn btn-primary mb-5 mb-md-0 me-3 waves-effect waves-light',
                'init' => 'function (api, node, config) {
                    $(node).removeClass("btn-secondary");
                }',
                'action' => 'function (e, dt, node, config) {
                    window.location = "' . route('materials.create') . '";
                }'
            ],
            [
                ...
                'action' => 'function (e, dt, node, config) {
                    $("#materials-table_filter input").val("").keyup();
